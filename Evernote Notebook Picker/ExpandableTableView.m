...
@property (weak) id<ExpandableTableViewDelegate, UIScrollViewDelegate> expandableDelegate;
...
- (NSString *)description {
	...
	for (NSInteger i = 0; i < sections1; i++) {
		...
		[s appendFormat:@"Section %lu has %lu rows\n", i, rows];
	}
...
- (void)expandSection:(NSUInteger)section {
  if ([_expandableDelegate respondsToSelector:@selector(tableView:shouldExpandSection:)]) {
    if (![_expandableDelegate tableView:self shouldExpandSection:section])
      return;
  }
...
- (void)contractSection:(NSUInteger)section {
  if ([_expandableDelegate respondsToSelector:@selector(tableView:shouldConstractSection:)]) {
    if (![_expandableDelegate tableView:self shouldConstractSection:section])
      return;
  }
...
- (void)tableView:(ExpandableTableView *)tableView didDeselectRowAtIndexPath:(NSIndexPath *)indexPath {
	if ([_expandedSectionIndexes containsIndex:indexPath.section] && indexPath.row != 0) {
		if ([_expandableDelegate respondsToSelector:@selector(tableView:didDeselectRowAtIndexPath:)]) {
...
- (CGFloat) tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
  if (indexPath.row != 0) {
    if ([_expandableDelegate respondsToSelector:@selector(tableView:heightForRowAtIndexPath:)]) {
      ...
      return [_expandableDelegate tableView:self heightForRowAtIndexPath:newIndexPath];
    }
  } else {
    if ([_expandableDelegate respondsToSelector:@selector(tableView:heightForGroupCellAtIndexPath:)]) {
      return [_expandableDelegate tableView:self heightForGroupCellAtIndexPath:indexPath];
    }
  }
...
// workaround of passing scrollview delegate
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
  if ([self.expandableDelegate respondsToSelector:@selector(scrollViewDidScroll:)]) {
    [self.expandableDelegate scrollViewDidScroll:self];
  }
}
...
- (NSIndexSet *)_insertRowsAtIndexPaths:(NSArray *)indexPaths withRowAnimation:(UITableViewRowAnimation)animation {
	...
	if ([correctedIndexPaths count]) {
		...
		NSLog(@"Adding %ld rows", [correctedIndexPaths count]);
	}
...
- (NSIndexSet *)_deleteRowsAtIndexPaths:(NSArray *)indexPaths withRowAnimation:(UITableViewRowAnimation)animation {
	...
	if ([correctedIndexPaths count]) {
		...
		NSLog(@"Removing %ld rows", [correctedIndexPaths count]);
	}
